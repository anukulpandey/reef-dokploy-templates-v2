...
chmod 700 /tmp/validator3

echo "🔑 Inserting session keys (babe, grandpa, im_online, authority_discovery)..."
$NODE_BIN key insert --base-path /tmp/validator3 --chain /tmp/local-chain-spec-raw-2.json \
...
echo
echo "🚀 Starting Validator Node (logs -> /tmp/validator3.log)..."
exec $NODE_BIN \
