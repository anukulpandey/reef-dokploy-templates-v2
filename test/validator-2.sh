...
chmod 700 /tmp/validator2

echo "🔑 Inserting session keys (babe, grandpa, im_online, authority_discovery)..."
$NODE_BIN key insert --base-path /tmp/validator2 --chain /tmp/local-chain-spec-raw-2.json \
...
echo
echo "🚀 Starting Validator Node (logs -> /tmp/validator2.log)..."
exec $NODE_BIN \
