...
reef-node build-spec --chain testnet-new --disable-default-bootnode \
  > /workspace/local-chain-spec.json
...
/workspace/update-spec.sh \
  ...
  --input /workspace/local-chain-spec.json \
  --output /workspace/local-chain-spec-updated.json

echo "👉 generating RAW spec"
reef-node build-spec \
  --chain /workspace/local-chain-spec-updated.json \
  --disable-default-bootnode --raw \
  > /workspace/local-chain-spec-raw.json

echo "🎉 RAW spec created at /workspace/local-chain-spec-raw.json"

echo
echo "Inserting keys for Validator 1..."

reef-node key insert --base-path /tmp/validator1 \
  --chain=/workspace/local-chain-spec-raw.json \
  --scheme Sr25519 \
  --suri "$V1_SEED//babe" \
  --key-type babe
reef-node key insert --base-path /tmp/validator1 \
...
reef-node key inspect-node-key --file /tmp/bootnode_node_key.txt > /workspace/bootnode_node_key.txt
